...
def _transform_weather_data(**context):
    ...
    returned_filename = s3_hook.download_file(
        filename, bucket_name=Variable.get("S3BucketName"), local_path="/tmp"
    )
    ...
    # Load it to S3
    s3_hook.load_file(
        filename=csv_filename_full_path,
        key=csv_filename,
        bucket_name=Variable.get("S3BucketName"),
    )
...
def _fetch_status_data(**context):
    ...
    response = requests.get(
        f"https://velib-metropole-opendata.smovengo.cloud/opendata/Velib_Metropole/station_status.json"
    )
...
with DAG(
    ...
    with TaskGroup(group_id="weather_branch") as weather_branch:
        fetch_weather_data = PythonOperator(
            task_id="fetch_weather_data", python_callable=_fetch_weather_data
        )

        transform_weather_data = PythonOperator(
            task_id="transform_weather_data", python_callable=_transform_weather_data
        )
    ...
    with TaskGroup(group_id="status_branch") as status_branch:
        fetch_status_data = PythonOperator(
            task_id="fetch_status_data", python_callable=_fetch_status_data
        )

        transform_status_data = PythonOperator(
            task_id="transform_status_data", python_callable=_transform_status_data
        )
